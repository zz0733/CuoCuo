...
playerCls.prototype.showPaiOnBegin = function(huaData){
    ...
    // }.bind(this), 800);
    // if(.length > 0){
    for(let i=0; i< huaData.showData.length; i++){
...
playerCls.prototype.resetFanPai = function(fanList = []){
...
playerCls.prototype.setDirection = function(direction){
    var GameDefine = require("mjGameDefine");
    var dirIndex   = 0;
    for(let k in GameDefine.DIRECTION_TYPE){
...
playerCls.prototype.addPaiOutList = function(paiList){
    for(let i =0; i<paiList.length; i++){
        let paiID = paiList[i];
        let pai = this.paiDataObj.createFullPai(paiID);
        pai.setShowType(GameDefine.PAISHOWTYPE.END);
...
playerCls.prototype.getPaiVoice = function(paiID){
    let commonType = {1:"tong", 2:"tiao", 3:"wan"};
    var voiceName = ""
    if(paiID < 40){
