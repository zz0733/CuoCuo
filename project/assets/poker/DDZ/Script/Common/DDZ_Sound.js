...
DDZ_Audio.playerOutSound = function (data) {
    this.bindPath();
    var soundPath = null;
    var typePath = ["", "s_", "dui_", "sanbudai", "sandaiyi", "sandaidui",
        "shunzi", "liandui", "feiji", "feiji", "feiji", "sidaidui", "sidaier",
        "zadan", "wangza"];
    var numStr = "";
    ...
    if (fun.db.getData('UserInfo').UserId == data.retMsg.userId) {
        cc.YL.DDZselfPlayerInfo.sex == 1
            ? soundPath = this._manPath + playArr[0]
            : soundPath = this._womanPath  + playArr[0]
    }
...
DDZ_Audio.playSpecialEffect = function (userId, str) {
    ...
    if (fun.db.getData('UserInfo').UserId == userId) {
        cc.YL.DDZselfPlayerInfo.sex == 1
            ? soundPath = this._manPath + str
            : soundPath = this._womanPath + str;
    }
    if (cc.YL.DDZrightPlayerInfo) {
        if (userId == cc.YL.DDZrightPlayerInfo.userId) {
            cc.YL.DDZrightPlayerInfo.sex == 1
                ? soundPath = this._manPath + str
                : soundPath = this._womanPath + str;
        }
    }
...
DDZ_Audio.playPass = function (userId) {
    ...
    if (fun.db.getData('UserInfo').UserId == userId) {
        cc.YL.DDZselfPlayerInfo.sex == 1
            ? soundPath = this._manPath + "pass_" + this.rangeInt(1, 3)
            : soundPath = this._womanPath + "pass_" + this.rangeInt(1, 3);
    }
    if (cc.YL.DDZrightPlayerInfo) {
        if (userId == cc.YL.DDZrightPlayerInfo.userId) {
            cc.YL.DDZrightPlayerInfo.sex == 1
...
DDZ_Audio.playWaring = function (userId,num) {
    ...
    if (fun.db.getData('UserInfo').UserId == userId) {
        cc.YL.DDZselfPlayerInfo.sex == 1
            ? soundPath = this._manPath + "last_" + num
            : soundPath = this._womanPath + "last_" + num;
    }
    if (cc.YL.DDZrightPlayerInfo) {
        if (userId == cc.YL.DDZrightPlayerInfo.userId) {
            cc.YL.DDZrightPlayerInfo.sex == 1
...
DDZ_Audio.playMsgMusic = function(userId,index){
    ...
    if (fun.db.getData('UserInfo').UserId == userId) {
        ...
        cc.YL.DDZselfPlayerInfo.sex == 1
            ? soundPath = this._manPath + pathName + index
            : soundPath = this._womanPath + pathName + index;
    }
    if (cc.YL.DDZrightPlayerInfo) {
        if (userId == cc.YL.DDZrightPlayerInfo.userId) {
            index >= 10 ? pathName = "male_talk0" :pathName = "male_talk00";
