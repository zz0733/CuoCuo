(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/poker/DDZ/Script/Game/DDZ_PokerTip.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '9a5cbwlJbZCbqGdtKg5jl82', 'DDZ_PokerTip', __filename);
// poker/DDZ/Script/Game/DDZ_PokerTip.js
...
DDZPokerTip.shunzi = function () {
    ...
    for (var i = 0; i < tempArr.length; i++) {
        for (var j = 4; j < 12; j++) {
            if (tempArr[i] + j == tempArr[i + j] && i + j <= tempArr.length - 1 && tempArr[i] <= 10 && tempArr[i + j] < 15) {
                this.shunZiArr.push({
                    min: tempArr[i],
                    max: tempArr[i + j],
                    len: j + 1
...
DDZPokerTip.doubleshunzi = function () {
    ...
    for (var i = 0; i < newArr.length; i++) {
        for (var j = 2; j < 10; j++) {
            if (newArr[i] + j == newArr[i + j] && i + j <= newArr.length - 1 && newArr[i] <= 12 && tempArr[i + j] < 15) {
...
DDZPokerTip.fly = function () {
    ...
    for (var i = 0; i < newArr.length; i++) {
        for (var j = 1; j < 7; j++) {
            if (newArr[i] + j == newArr[i + j] && i + j <= newArr.length - 1 && newArr[i] <= 13) {
                this.flyArr.push({
                    min: newArr[i],
                    max: newArr[i + j],
                    len: j + 1
...
DDZPokerTip.upPoker = function (pokerNum) {
    ...
    for (var j = 0; j < pokerRoot.children.length; j++) {
        if (pokerNum == pokerRoot.children[j].getComponent("DDZ_Poker").pokerNum && pokerRoot.children[j].y == 0 && times == 0) {
        ...
        case 10:
            {
                ...
                for (var i = 0; i < this.flyArr.length; i++) {
                    if (this.flyArr[i].min > this.findOutCardList(this.lastPokerList, 3) && this.flyArr[i].len * 3 + this.flyArr[i].len * 2 == this.lastPokerNum) {
                        var tempArr = [];
        ...
        case 9:
            {
                ...
                for (var i = 0; i < this.flyArr.length; i++) {
                    if (this.flyArr[i].min > this.findOutCardList(this.lastPokerList, 3) && this.flyArr[i].len * 3 + this.flyArr[i].len == this.lastPokerNum) {
                        var tempArr = [];
                        for (var k = 0; k < this.flyArr[i].len; k++) {
                            tempArr.push(this.flyArr[i].min + k);
                            tempArr.push(this.flyArr[i].min + k);
        ...
        case 8:
            {
                var tipFlyCardArr = [];
                for (var i = 0; i < this.flyArr.length; i++) {
                    if (this.flyArr[i].min > this.findOutCardList(this.lastPokerList, 3) && this.flyArr[i].len == this.lastPokerNum / 3) {
                        var tempArr = [];
                        for (var k = 0; k < this.flyArr[i].len; k++) {
                            tempArr.push(this.flyArr[i].min + k);
                            tempArr.push(this.flyArr[i].min + k);
...
cc._RF.pop();
        ...
        //# sourceMappingURL=DDZ_PokerTip.js.map
